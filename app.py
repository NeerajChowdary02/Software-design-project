...
# code used for testing purposes to connecting application to the database on mac
#app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:psql123@localhost/john_database'

#app.config['SQLALCHEMY_DATABASE_URI'] = 'DBtype://username:DBpassword@localhost/DBname'
# swap out dbURI
...
#code used for testing purposes while connecting database to application
# db = SQLAlchemy()
#
...
def get_db():
    if 'db' not in g:
        g.db = app.config['postgreSQL_pool'].getconn() #postgreSQL_pool
    return g.db
...
def close_conn(e):
    db = g.pop('db', None)
    if db is not None:
        app.config['postgreSQL_pool'].putconn(db) #postgres_pool
...
# code for the profile page of the customer, saves in information of the customer such as name, address etc.
# checj here for error in any case the client profile page gives errors for this page in virtual environment
...
# This is the code for the fuel qote form where users can request for gallons
# Must finish profile form to get a quote
def fuel_quote_form():
    if request.method == "POST":
        ...
        price_p_gal = price_module.Pricing_module().calcPrice(session['state'],\
                        session['hist'], gallons_requested) #new change
